...
export const getAllProductsData = async (req, res, next) => {
    ...
    try {
        ...
        const { rows: productsData} = await client.query("SELECT p.id AS producto_id, p.tipo_id AS tipo, p.nombre, p.url_imagen, COALESCE( ARRAY_AGG(jsonb_build_object('codigo', pub.codigo, 'variante', pub.variante, 'plataforma', pub.plataforma_id, 'id', pub.id, 'active', pub.active, 'full', pub.full_bolean, 'medellin', pub.medellin, 'fijo', CASE WHEN pf.publicacion_id IS NOT NULL THEN true ELSE false END ) ) FILTER (WHERE pub.id IS NOT NULL), '{}'::jsonb[] ) AS publicaciones, COALESCE( jsonb_agg(DISTINCT replace(sku.sku, 'caracter_problematico', '')) FILTER (WHERE sku.sku IS NOT NULL),  '[]'::jsonb ) AS skus FROM productos p LEFT JOIN publicaciones pub ON p.id = pub.producto_id LEFT JOIN publicaciones_fijas pf ON pub.id = pf.publicacion_id LEFT JOIN sku_producto sku ON p.id = sku.producto_id GROUP BY p.id, p.nombre, p.url_imagen ORDER BY p.id");
